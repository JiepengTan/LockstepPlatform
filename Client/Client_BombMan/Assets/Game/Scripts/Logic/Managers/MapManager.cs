...
namespace Lockstep.Game {
    ...
    public partial class MapManager : IMapService, ITimeMachine {
        ...
        private IGroup<GameEntity> _bombGroup;
        public bool HasBomb(Vector2Int pos){
            if (_bombGroup == null) {
                _bombGroup= _gameContext.GetGroup(GameMatcher.AllOf(GameMatcher.LocalId, GameMatcher.Bomb));
            }

            foreach (var bomb in _bombGroup.GetEntities()) {
                if (bomb.pos.value.Floor() == pos) {
                    return true;
                }
            }
            return false;
