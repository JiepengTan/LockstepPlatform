...
namespace Lockstep.Server.Game {
    ...
    public interface IGame : IRecyclable {
        ...
        //room life cycle
        void DoStart(IGameServer server, int type, int roomId, int size, string name);
        ...
        void BorderMessageTcp(EMsgSC type, BaseFormater info);
    ...
    public class BaseGame : IGame {
        ...
        public int CurPlayerCount => _allPlayers.Count;
        ...
        public void DoStart(IGameServer server, int type, int roomId, int size, string name){
            this.RoomId = roomId;
        ...
        public void BorderMessage(EMsgSC type, BaseFormater data){
        ...
        public void SendTo(Player player, EMsgSC type, ISerializable body, bool isNeedDebugSize = false){
            var writer = new Serializer();
            writer.PutByte((byte) (short) type);
        ...
        public void OnPlayerEnter(Player player){
            if (State == EGameState.Idle) {
                State = EGameState.WaitingToPlay;
            }

            if (State != EGameState.WaitingToPlay) return;
            if (_allPlayers.Contains(player)) {
                Debug.LogError("Player already exist" + player.UserId);
                return;
            }

            Debug.Log($"Player{player.UserId} Enter room {RoomId}");
            //player.room = this;
            _allPlayers.Add(player);
        }

        public void OnPlayerReady(Player tempplayer){
            Debug.Log("OnPlayerReady " + tempplayer.UserId);
        ...
        public void OnPlayerLeave(Player player){
            RemovePlayer(player);
            _playerId2LocalId.Remove(player.UserId); //同时还需要彻底的移除记录 避免玩家重连
            Debug.Log($"Player{player.UserId} OnPlayerLeave room {RoomId}");
        }
        ...
        public void OnReconnect(Player player){
            player.LocalId = _playerId2LocalId[player.UserId];
            _localId2Player[player.LocalId] = player;
            if (_allPlayers.Contains(player)) {
                Debug.LogError("Duplicate add players" + player.UserId);
                return;
            }
        ...
        public void OnDisconnect(Player player){
            Debug.Log($"Player{player.UserId} OnDisconnect room {RoomId}");
            RemovePlayer(player);
        }
        ...
        public void StartGame(){
            ...
            foreach (var player in _allPlayers) {
                ...
                _playerId2LocalId[player.UserId] = localId;
        ...
        void OnNet_LoadingProgress(Player player, BaseFormater data){
            if (State != EGameState.PartLoading) return;
            ...
            var retmsg = new Msg_G2C_LoadingProgress();
        ...
        private void StartSimulationOnConnectedPeers(){
            ...
            var msg = CreateStartGame();
            foreach (var player in _allPlayers) {
                SendTo(player, EMsgSC.G2C_GameStartInfo, msg);
            }
        }

        Msg_G2C_GameStartInfo CreateStartGame(){
            return new Msg_G2C_GameStartInfo {
                RoomId = RoomId,
                Seed = seed,
