...
namespace Lockstep.Logic.Server {
    public class Lobby : ILobby {
        ...
        public const byte MAX_HANDLER_IDX = (byte) EMsgCS.EnumCount;
        public const byte INIT_MSG_IDX = (byte) EMsgCS.C2L_InitMsg;
        ...
        public void DoStart(int port){
            NetMsgExtension.RegisterAllNetMsg();
            RegisterMsgHandlers();
        ...
        public IRoom CreateRoom(int type, Player master, string roomName){
            ...
            IRoom room = new Room(); //TODO should load dll 
        ...
        public void realOnDataReceived(int netID, byte[] data){
            ...
            {
                if (CheckMsg(reader, netID, playerID, msgType == (byte) EMsgCS.C2L_InitMsg, out var player)) return;
        ...
        private void RegisterMsgHandlers(){
            RegisterNetMsgHandler(EMsgCS.C2L_InitMsg, OnMsg_InitMsg);
            RegisterNetMsgHandler(EMsgCS.C2L_JoinRoom, OnMsg_JoinRoom);
            RegisterNetMsgHandler(EMsgCS.C2L_CreateRoom, OnMsg_CreateRoom);
            RegisterNetMsgHandler(EMsgCS.C2L_LeaveRoom, OnMsg_LeaveRoom);
            RegisterNetMsgHandler(EMsgCS.C2L_RoomMsg, OnMsg_RoomMsg);
        }

        private void RegisterNetMsgHandler(EMsgCS type, DealNetMsg func){
        ...
        private void SendReqInit(Player player){
            player.Send(EMsgCS.L2C_ReqInit, new ReqInit() {playerId = player.PlayerId});
        }
